...
public class ApplicationHandler {
    ...
    @Operation(operationId = "save", summary = "Register a new request", description = "Register a new request with your basic personal information", tags = {
    ...
    public Mono<ServerResponse> save(ServerRequest serverRequest) {
        ...
        if (documentToken == null || roleToken == null || rawToken == null) {
                return ServerResponse.status(HttpStatus.UNAUTHORIZED)
                        ...
                        .bodyValue(ResponseUtil.responseError("Unauthorized: Missing or invalid token"));
        }

        if (!"CLIENT".equalsIgnoreCase(roleToken)) {
                return ServerResponse.status(HttpStatus.FORBIDDEN)
                        .contentType(MediaType.APPLICATION_JSON)
                        .bodyValue(ResponseUtil.responseError("Only CLIENT role can create loan requests"));
        }
        ...
        return serverRequest.bodyToMono(ApplicationRequest.class)
                ...
                .flatMap(request -> {
                        ...
                        return ServerResponse.status(HttpStatus.FORBIDDEN)
                                .contentType(MediaType.APPLICATION_JSON)
                                .bodyValue(ResponseUtil.responseError("Unauthorized: Document does not match token"));
                        }

                        return applicationUseCase.execute(mapper.toDomain(request), rawToken.replace("Bearer ", ""))
                                .map(mapper::toResponse)
                                .flatMap(response -> ServerResponse.status(HttpStatus.CREATED)
                                        .contentType(MediaType.APPLICATION_JSON)
                                        .bodyValue(ResponseUtil.responseCreated(response)));
                });
        }
