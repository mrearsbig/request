...
import com.mrearsbig.api.dto.ApplicationUpdate;
...
public class ApplicationHandler {
    ...
    public Mono<ServerResponse> update(ServerRequest serverRequest) {
        ...
        return serverRequest.bodyToMono(ApplicationUpdate.class)
                .flatMap(validatorUtil::validate)
                .flatMap(request -> {
                    if (request.status() == 1) {
                        return applicationUseCase.approve(id, rawToken.replace("Bearer ", ""));
                    } else if (request.status() == 3) {
                        return applicationUseCase.reject(id, rawToken.replace("Bearer ", ""));
                    } else {
                        return Mono.error(new IllegalArgumentException("Invalid status value"));
                    }
                })
                .map(mapper::toResponse)
